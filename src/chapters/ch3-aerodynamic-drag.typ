...
#grid(
  ...
  column-gutter: 1em,
  row-gutter: 0.65em,
  $y = 0 colon quad u = v = 0$,
  [(both the tangential and the normal velocity components must vanish at the wall)],
  $y = infinity colon quad u = U_(infinity)(x,t)$,
  [(the tangential velocity component must be identical to that computed from potential-flow theory at large distances from the wall)],
)
...
// === page 62 ===
then seen to reduce to (40) $u (partial u)/(partial x) + v (partial u)/(partial y) = nu (partial^2 u)/(partial y^2)$ (41) $(partial u)/(partial x) + (partial v)/(partial y) = 0$ and the boundary conditions become $y = 0: u = v = 0$ $y = infinity: u = U_(infinity)$ (note that $U_(infinity)$ is now independent of x and t) 
...
// === page 64 ===
$ rho rho'' + 2 rho''' = 0 $
with the boundary conditions (52) $eta = 0 : f = 0 ; f' = 0$ (53) $eta = infinity : f' = 1$ 
...
Now from equation (65) it will be recalled that (72) $U_(infinity)^2 theta = integral_(y=0)^(infinity) u (U_(infinity) - u) dif y$ This expression is identical to the integral appearing in equations (70) and (71); hence we have (73) $D = b delta U_(infinity)^2 theta$ 
...
Approximate values of B for several possible values of $R_(c r i t)$ are listed below @boundary-layer-theory:   Rcrit 3 \times 10^5 5 \times 10^5 1 \times 10^6 3 \times 10^6   B 1050 1700 3300 8700   

In order to apply equation (101) in a particular case, the value of $R_(c r i t)$ must be known.
...
Data taken by Douglas J. @drag-coefficient-measurements indicate that placing the launch lug in one of the joints between the model's fins and its body can substantially decrease the launch lug drag increment.
...
For the purpose of these calculations, the planform area of a fin is considered to be the sum of its actual, exposed planform area and the planform area of its imaginary extension into the body tube.
...
and finally (209) $D = [ ((pi/8) rho nu^2)/((l_b/d_m)^2) ] C_D R_e^2$ 

Since $C_D$ is known as a function of $R_e$ from a plot like that of @fig:gcr-drag-vs-reynolds, compiled from Datcom calculations, it is possible to determine the behavior of the drag force as a function of $R_e$ using equation (209).
